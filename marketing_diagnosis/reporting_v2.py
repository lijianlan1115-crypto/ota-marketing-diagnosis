...
def _promotion_blocks(metrics: dict[str, Any]) -> str:
    ...
    for item in _channel_items(metrics):
        ...
        rows = [
            ["推广订单金额", "未获取", "推广订单金额", "推广产出"],
            ["推广花费", "未获取", "推广花费", "推广成本"],
            ["ROI", "未获取", "订单金额 ÷ 推广花费", "推广效率"],
            ["活动数", _num(pr.get("activity_count")), "活动覆盖", "活动基础"],
            ["活动商品明细", _num(pr.get("activity_product_count")), "活动商品数", "商品覆盖"],
            ["推广明细", detail, "推广明细完整度", "影响封顶规则"],
        ]
    ...
    return "".join(panels) + f"<div class='meta'>当前预留推广效率口径：{_e(missing)}。字段未接入时，只能判断活动覆盖，不能判断投放 ROI。</div>"
...
def _page_blocks(metrics: dict[str, Any]) -> str:
    ...
    for item in _channel_items(metrics):
        ...
        rows = [
            ["图片质量", "未知", "页面基础", "页面基础"],
            ["视频状态", "未知", "内容完整度", "内容完整度"],
            ["房型卖点", product_status, "商品名/房型映射", "转化支撑"],
            ["入口标签", "未知", "搜索和入口质量", "搜索入口"],
        ]
...
def _reputation_blocks(metrics: dict[str, Any]) -> str:
    ...
    for item in _channel_items(metrics):
        ...
        rows = [
            ["平台评分", _num(r.get("rating_avg"), 2), "平台评分", "信任锚点"],
            ["总评价数", _num(total), "点评数量", "评价规模"],
            ["好评数", _num(positive), "总评 - 差评", "正向反馈"],
            ["差评数", _num(negative), "差评数", "口碑风险"],
            ["差评率", _pct(r.get("negative_review_rate")), "差评数 ÷ 总评", "口碑风险"],
            ["未回复评价", _num(r.get("unreplied_review_count")), "未回复评价数", "服务响应"],
        ]
...
<div class='layout'><nav class='side'><a href='#overview'>顶部总览</a><a href='#trend'>月度趋势</a><a href='#modules'>模块诊断</a><a href='#metrics'>经营指标</a><a href='#funnel'>流量漏斗</a><a href='#promotion'>推广效率</a><a href='#page'>页面入口</a><a href='#reputation'>口碑分析</a><a href='#channels'>分渠道指标</a><a href='#price'>价格竞品</a><a href='#missing'>补采提示</a><a href='#actions'>整改任务</a></nav><main>

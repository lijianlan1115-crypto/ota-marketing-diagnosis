...
def _analysis(title: str, paragraphs: list[str], open_: bool = True) -> str:
...
def _trend_svg(monthly: list[dict[str, Any]]) -> str:
    ...
    svg = f"<svg viewBox='0 0 {width} {height}' style='width:100%;display:block;height:auto' aria-label='月度经营趋势'>{''.join(grid)}{''.join(labels)}{line_for('revpar', '#2563eb')}{line_for('adr', '#16a34a')}</svg>"
...
def _trend_rows(monthly: list[dict[str, Any]]) -> list[list[Any]]:
    rows = [[item.get("month"), _money(item.get("adr")), _pct(item.get("occupancy_rate")), _money(item.get("revpar")), _money(item.get("room_revenue"))] for item in monthly or []]
    return rows or [["无", "未获取", "未获取", "未获取", "未获取"]]


def _channel_rows(metrics: dict[str, Any]) -> list[list[Any]]:
    ...
    return rows or [["无", "未获取", "未获取", "未获取", "未获取", "未获取", "未获取"]]
...
def _metric_rows(metrics: dict[str, Any]) -> list[list[Any]]:
    ...
    items = [
        ("RevPAR（每间可售房收入）", _money(op.get("revpar")), "收益锚点"),
        ("ADR（平均房价）", _money(op.get("adr")), "价格水平"),
        ("出租率", _pct(op.get("occupancy_rate")), "经营效率"),
        ("门店收入", _money(op.get("room_revenue")), "经营结果"),
        ("曝光量", _plain_num(funnel.get("exposure")), "流量入口"),
        ("浏览量（UV）", _plain_num(funnel.get("views")), "详情页访问"),
        ("支付订单", _plain_num(funnel.get("paid_orders")), "转化结果"),
        ("浏览→支付转化率", _pct(funnel.get("payment_conversion_rate")), "转化结果"),
        ("商品最低价", _money(price.get("min_price")), "价格梯度"),
        ("活动数", _plain_num(promo.get("activity_count")), "推广覆盖"),
        ("近期周边活动", _plain_num(events.get("upcoming_60d_count")), "需求事件"),
        ("平台评分", _plain_num(rep.get("rating_avg"), 2), "信任锚点"),
        ("差评率", _pct(rep.get("negative_review_rate")), "口碑风险"),
    ]
    rows = []
...
def _module_analysis(module_id: str, label: str, rate: float, status: str, metrics: dict[str, Any]) -> str:
    ...
    rep = metrics.get("reputation") or {}
    advice = {
        "M01": f"当前 RevPAR {_money(op.get('revpar'))}，ADR {_money(op.get('adr'))}，出租率 {_pct(op.get('occupancy_rate'))}。出租率按所选周期聚合口径计算。",
        "M02": f"曝光 {_plain_num(funnel.get('exposure'))}，浏览 {_plain_num(funnel.get('views'))}。继续拆解曝光来源、自然流量占比和竞争圈排名。",
        "M03": f"浏览→支付转化率 {_pct(funnel.get('payment_conversion_rate'))}。重点排查曝光到浏览、浏览到支付的断点。",
        "M04": "梳理全日房、钟点房、团购和活动价，保持清晰价格梯度。",
        "M05": "当前已接入活动覆盖，但推广成本、点击、推广订单、推广收入、ROI 字段仍需补齐。",
        "M06": "优化首图、视频、房型卖点、标签入口和权益配置。",
        "M07": f"平台评分 {_plain_num(rep.get('rating_avg'), 2)}，差评率 {_pct(rep.get('negative_review_rate'))}。将好评关键词反哺页面卖点。",
...
<html lang='zh-CN'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1'><title>酒店 OTA 全面诊断报告</title><style>{HTML_STYLE}</style></head>
<body>
  <header class='app-header'><div class='header-inner'><div class='title-block'><h1>酒店 OTA 全面诊断报告</h1><p>{_esc(title_meta)}</p></div><div class='actions'><div class='channel-selector'><label>选择渠道：</label><select id='channelSelector'><option value='all'>全部渠道</option><option value='meituan'>美团</option><option value='ctrip'>携程</option></select></div><button class='btn primary' onclick='window.print()'>导出报告</button></div></div></header>
  <div class='layout'><nav class='sidebar dashboard-only'><a href='#overview'>顶部总览卡片</a><a href='#trend'>月度趋势图</a><a href='#modules'>模块诊断</a><a href='#metrics'>经营指标</a><a href='#funnel'>流量漏斗</a><a href='#channels'>分渠道指标</a><a href='#missing'>补采提示</a><a href='#actions'>诊断结论</a></nav><main>
    ...
    <section id='metrics'><div class='section-head'><div><h2>经营指标</h2><p>关键经营数据一览</p></div></div><div class='section-body'>{_table(['指标','当前值','口径','状态'], _metric_rows(metrics))}{_analysis('指标解读', [f"RevPAR {_money(operating.get('revpar'))} = ADR {_money(operating.get('adr'))} × 出租率 {_pct(operating.get('occupancy_rate'))}。", f"出租率使用所选周期内总售出间夜 / 总可售房晚；如果日表缺少出租率字段，会用 room_nights 或 sold_rooms 除以 room_count 计算。", f"曝光→浏览转化率 {_pct(funnel.get('exposure_to_view_rate'))}，浏览→支付转化率 {_pct(funnel.get('payment_conversion_rate'))}。"] )}</div></section>
    ...
    <section id='channels'><div class='section-head'><div><h2>分渠道指标</h2><p>各 OTA 平台独立指标概览；当前按真实可用平台数据展示。</p></div></div><div class='section-body'>{_table(['渠道','曝光','浏览','支付订单','转化率','评价数','评分'], _channel_rows(metrics))}</div></section>

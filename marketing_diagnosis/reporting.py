...
def _trend(monthly: list[dict[str, Any]]) -> str:
    if not monthly:
        return "暂无月度趋势数据。"
    values = [x.get("revpar") for x in monthly if _n(x.get("revpar")) is not None] + [x.get("adr") for x in monthly if _n(x.get("adr")) is not None]
    if not values:
        return "趋势字段未获取。"
    w,h,l,r,t,b=700,260,60,680,18,220
    mx, mn = max(values)*1.1, min(0,min(values)*0.9)
    span=max(mx-mn,1)
    def xx(i): return l+(r-l)*(i/max(1,len(monthly)-1))
    def yy(v): return b-(float(v or 0)-mn)/span*(b-t)
    def line(key,color):
        pts=[]; cir=[]
        for i,item in enumerate(monthly):
            v=_n(item.get(key))
            if v is None: continue
            x,y=xx(i),yy(v); pts.append(f"{x:.1f},{y:.1f}"); cir.append(f"<circle cx='{x:.1f}' cy='{y:.1f}' r='4' fill='{color}'/>")
        return "" if not pts else f"<polyline points='{' '.join(pts)}' fill='none' stroke='{color}' stroke-width='2.5'/>"+"".join(cir)
    labels="".join(f"<text x='{xx(i):.1f}' y='238' text-anchor='middle' style='font-size:10px;fill:#667085'>{_e(item.get('month'))}</text>" for i,item in enumerate(monthly))
    grid="".join(f"<line x1='{l}' y1='{t+(b-t)*i/4:.1f}' x2='{r}' y2='{t+(b-t)*i/4:.1f}' stroke='#e5e7eb' stroke-dasharray='4,3'/>" for i in range(5))
    return f"<svg viewBox='0 0 {w} {h}' style='width:100%;height:auto'>{grid}{labels}{line('revpar','#2563eb')}{line('adr','#168a4a')}</svg><div class='meta'>蓝线 RevPAR，绿线 ADR</div>"
...
def _module_blocks(metrics: dict[str, Any], ms: list[dict[str, Any]]) -> str:
    ...
    html_parts=[]
    for source,title,mids in groups:
        cards="".join(_module_card(_src_module(source,mid,metrics,ms)) for mid in mids)
        html_parts.append(f"<div class='source-block' data-module-source='{_e(source)}'><div class='source-title'>{_e(title)} <span class='badge neutral'>{_e(source)}</span></div><div class='source-grid'>{cards}</div></div>")
...
<section class='card' id='trend' data-module-source='pms'><div class='head'><div><h2>月度经营趋势</h2><p>PMS 经营趋势</p></div></div><div class='body'><div class='two'><div class='subpanel'><h3>趋势图</h3><div class='subpanel-content'>{_trend(op.get('monthly_trend') or [])}</div></div><div class='subpanel'><h3>月度经营数据</h3><div class='subpanel-content'>{_table(['月份','ADR','出租率','RevPAR','门店收入'],_trend_rows(op.get('monthly_trend') or []))}</div></div></div></div></section>
